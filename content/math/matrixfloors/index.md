+++
title = "Solving a Matrix with Legendre Floors"
description = "A neat application of number theory to a linear algebra composition."
date = "2026-02-24"
+++
{{< katex >}}
...
2.  Prove/disprove: \(\alpha < 3930!\)

3.  Express \(\alpha\) in terms of factorials and prime-counting or divisor functions.
...
2.  Prove/disprove: \(\beta < 3930!\)

## Eigenvalues

1.  If possible, find the sum of the eigenvalues of \(A\) (truncated if needed).

2.  If possible, find the product of the eigenvalues of \(A\) (truncated if needed).
...
1.  **\(\alpha\) converges:** Let \(k\) be the first prime larger than 3930. Then,
$$
...
2.  **Express \(\alpha\) in terms of factorials and prime-counting or divisor functions.** Notice that since every column contains the Legendre sum for prime \(p_n\): \(\boxed{\Omega(3930!)}\)

3.  **Prove/disprove: \(\alpha < 3930!\)** Observe that 2 would have the highest exponent in \(\Omega(3930!)\), since it is the smallest prime. Using geometric series for an upper bound, we can see \(3930>(\text{exponent of 2})\).
$$
    \begin{aligned}
    \sum_{n=1}^{\infty}\left\lfloor \frac{3930}{2^n} \right\rfloor
    &< \sum_{n=1}^{\infty} \left( \frac{3930}{2^n} \right) \\
    &< 3930
    \end{aligned}
$$
...
1.  **\(\beta\) converges:** There exist infinitely many zero columns; the product of all entries must then be \(\boxed{0}\).

2.  **\(\beta < 3930!\), as \(0<3930!\)**

## Eigenvalues

1.  **If possible, find the sum of the eigenvalues of \(A\) (truncated if needed).** Recall that the sum of the eigenvalues is equal to the trace of the matrix. Adding the diagonal entries:
$$
    \begin{aligned}
